...
exports.addPercentageLimit = async (req, res) => {
  try {
    ...
    if (percentageLimit != null || percentageLimit != undefined) {
      res.status(400).json({
        message: "percentage limit already exist for this lotteryCategory",
      });
    }
...
exports.updatePercentageLimit = async (req, res) => {
  try {
    const { id } = req.params;
    const subAdminId = mongoose.Types.ObjectId(req.userId);
    const limit = await LimitPercentage.findByIdAndUpdate(id, req.body, {
      new: true,
    });
...
exports.getPercentageLimit = async (req, res) => {
  try {
    ...
    if (limits.length == 0) {
      res
        .status(404)
        .status({ message: "No percentageLimit is found for this Admin" });
    }
...
exports.deletePercentageLimit = async (req, res) => {
  try {
    ...
    // Attempt deletion
    const percentagelimit = await LimitPercentage.findByIdAndDelete(id);
    if (!percentagelimit) {
      return res.status(404).send({ error: "Percentage limit not found" });
    }

    res.status(200).send(percentagelimit);
  } catch (error) {
    res.status(500).send({ error: "Failed to delete the percentage limit" });
  }
};
