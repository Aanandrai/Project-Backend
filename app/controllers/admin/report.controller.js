...
//getSells repost 13/10
exports.getSaleReports= async (req, res) => {
  try {
    const fromDate = req.query.fromDate;
    const toDate = req.query.toDate;
    const subAdminId = req.query.subAdmin; // optional subAdmin filter
    const adminId = mongoose.Types.ObjectId(req.userId); // Admin ID from userId

    // Step 1: Get sub-admins associated with the admin
    const subAdmins = await User.find({ managerId: adminId }, { _id: 1 });
    const subAdminIds = subAdmins.map((item) => item._id);

    // Step 2: Prepare the seller query based on selected subAdminId
    let sellerIds = [];
    let sellerQuery = {};
    if (subAdminId) {
      // If a specific sub-admin is selected, filter sellers by that sub-admin
      sellerIds = await User.find({ subAdminId: subAdminId }, { _id: 1 });
      sellerQuery = { $in: sellerIds.map((item) => item._id) };
    } else {
      // If no specific sub-admin is selected, get sellers from all sub-admins
      sellerIds = await User.find({ subAdminId: { $in: subAdminIds } }, { _id: 1 });
      sellerQuery = { $in: sellerIds.map((item) => item._id) };
    }
    ...
    const result = await Ticket.aggregate([
      {
        $group: {
          _id: "$isDelete", // Group by isDelete field
          userDetails: { $push: "$$ROOT" } // Push all documents in each group into the userDetails array
        }
      }
    ...
    //                 { $in: ["$subAdmin", subAdminIds] }, // Correct usage
    //               ],
    //             },
    //           },
    //         },
